...
future_volatility_20 = (df.groupby("ticker")["future_return"]
...
# threshold = future_volatility_20.quantile(0.75)
# df["high_risk"] = (
#     future_volatility_20 >= threshold
# ).astype(int)

VOL_THRESHOLD = 0.025
df["high_risk"] = (future_volatility_20 >= VOL_THRESHOLD).astype(int)

df = df.dropna().reset_index(drop=True)
x = df.drop(columns=["future_return", "ticker", "date", "high_risk"])
y = df['high_risk']
...
for col in inf_cols:
    print(col)
    print(x[col][np.isinf(x[col])].head())

print(np.nanmax(np.abs(x.values)))
...
if best_model is None:
    print("LGBM remains the best model. Rebuilding it using the best parameters...")
    ...
    print("LGBM model rebuilt successfully.")

for name, r in results.items():
    print(
    f"{name:10s} "
    f"CV: {r['cv_f1']:.4f} "
    f"Holdout: {r['accuracy']:.4f}"
)
...
joblib.dump(best_model, "models/best_classifier_final_model.pkl")
joblib.dump(scaler, "models/final_scaler.pkl")
joblib.dump(VOL_THRESHOLD,"models/volatility_threshold.pkl")
joblib.dump(
    list(x.columns),
    "models/classificaion_feature_columns.pkl"
)
pd.DataFrame(results).T.to_csv(
    "models/classifier_results.csv"
)
